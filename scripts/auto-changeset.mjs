 ...
 */
//
// post-commit hook (lefthook). Reads the just-made commit, and if it's a
// consumer-visible conventional commit (feat / fix / perf / breaking), generates
// a matching .changeset/auto-*.md and AMENDS it into the same commit.
//
// Why post-commit + amend (not prepare-commit-msg): with `git commit -m`, git
// snapshots the index to build the commit tree BEFORE prepare-commit-msg runs —
// a `git add` from inside that hook updates the index but misses the already-built
// tree, so the changeset lands in the NEXT commit. post-commit + amend guarantees
// the changeset and its originating diff share one hash.
//
// Skipped on dev/master and on merge/rebase via lefthook config.
...
// --diff-filter=A: only newly-added paths. A commit that merely *touches* a
// pre-existing .changeset/auto-*.md must still generate its own changeset.
