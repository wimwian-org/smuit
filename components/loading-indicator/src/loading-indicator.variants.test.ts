...
test('footprint + fill are CSS-driven, so slot classes are stable across variant/size/tint/shape', () => {
    ...
    expect(loadingIndicator({ containerShape: 'cookie' }).root()).toBe(base);
});
    ...
    for (const variant of ['uncontained', 'contained'] as const) {
        for (const size of ['sm', 'md', 'lg'] as const) {
            for (const tint of ['neutral', 'primary', 'secondary', 'tertiary'] as const) {
                for (const containerShape of ['rounded', 'squircle', 'cookie'] as const) {
                    const s = loadingIndicator({ variant, size, tint, containerShape });
